...
const Home = async () => {
  ...
  return (
    <div>
      <h1
        className={`${lato.className} tracking-widest text-lg text-[#111111] text-center`}
      >
        Highlights.
      </h1>
      <div className="columns-1 gap-4 grid justify-items-center mt-8">
        {imageUrls.map((url: string, index: number) => (
          <div
            className={`${
              index < imageUrls.length - 1 ? 'mb-8' : ''
            } w-full h-auto md:w-[calc(700px-5vw)] md:max-w-[600px]`}
            key={index}
          >
            <Image
              placeholder="blur"
